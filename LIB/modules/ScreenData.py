...
class MainScreen:
        ...
        if screen == 'main_main':
            if key != 'exit':
                ...
                if self.menuitems['exit'].group == 'from_game_return_to_menu':
                    ...
                    self.pics.update({'background': Sprite(
                        (((Globals.RESOLUTION[0] - 1820) / 2) - 1820, self.pics['gamebackground'].pos[1]),
                        Globals.PICS['background'], 50),
                                      'logo': Globals.PICS['logo'],
                    ...
                    self.labels.update({'APPNAME': AlphaText('PyMonopoly', 'APPNAME'),
                                        'APPVERSION': AlphaText(Globals.TRANSLATION[4] + Globals.VERSION, 'APPVERSION'),
                                        'resources': AlphaText('Thanks to: freemusicarchive.org, openclipart.org',
                                                               'authors', 0),
                                        'authors': AlphaText('Anthony Samartsev & Michael Mozhaev, 2014-2016',
                                                             'authors', 1)})
            ...
            self.menuitems = {'uniform_build': MenuItem(
                u'‹ ' + Globals.TRANSLATION[18 - int(Globals.TEMP_VARS['build_style'])] + u' ›',
                'main_new_uniform_build', 'main_settings_left_MI', 0),
            ...
            for i in range(len(Globals.PLAYERS)):
                ...
                self.menuitems.update({'player_' + Globals.PLAYERS[i].name: MenuItem(u'●',
                                                                                     'pl_info_tab_' + Globals.PLAYERS[
                                                                                         i].name, 'pl_info_tab', i)})
                self.labels.update({'money_player_' + Globals.PLAYERS[i].name: AlphaText(str(Globals.PLAYERS[i].money),
                                                                                         'pl_money_info', i)})
            self.labels.update({'bank_property1': AlphaText(Globals.TRANSLATION[104], 'bank_property1'),
                                ...
                                'bank_property3': AlphaText(str(Globals.TEMP_VARS['bank_property'][0]),
                                                            'bank_property3'),
                                'bank_property4': AlphaText(u'❖', 'bank_property4'),
                                'bank_property5': AlphaText(str(Globals.TEMP_VARS['bank_property'][1]),
                                                            'bank_property5')})
            self.objects['gamefield'].change_new_pos((-1820, 0))
            self.objects['cur_turn_highlighter'] = CurTurnHighlighter(self.menuitems)
            self.pics.update({'gamebackground': Sprite((self.pics['background'].pos[0] + 1820, -130),
                                                       Globals.PICS['background'], 50),
                              'order': ['background', 'gamebackground', 'logo']})
            for key in ('background', 'gamebackground', 'logo'):
                self.pics[key].new_pos = (self.pics[key].new_pos[0] - 1820, self.pics[key].new_pos[1])
    ...
    def events(self, cur_key):
        for e in pygame.event.get():
            if e.type == pygame.MOUSEBUTTONDOWN and e.button == 1 and cur_key:
                ...
                elif e.key in (pygame.K_LEFT, pygame.K_RIGHT) and self.menuitems and self.cursor and 'SELECTOR' in \
                        self.menuitems[self.cursor.active_key].type:
                    self.menuitems[self.cursor.active_key].selector.keypress(e.key)
                elif self.menuitems and ('main_new_edit_player' in self.menuitems['exit'].type or self.menuitems[
                    'exit'].type == 'main_settings_player'):
    ...
    def input_handling(self, e, KEY, max_length):
        ...
        elif len(self.labels[KEY].symbols) < max_length and (
                KEY == 'name_MI' or e.unicode in ''.join([str(i) for i in range(10)])):
            self.labels[KEY].update_text(self.labels[KEY].symbols + e.unicode, False)
        if 'property_management_input' in self.labels.keys() or check_substring_in_dict_keys(self.labels,
                                                                                             'trading_input'):
    ...
    def action_call(self, key):
        # self.DEBUGGER_show_TEMP_VARS_keys()
        if Globals.TRANSLATION[100] in self.menuitems[key].text.symbols and ((key == 'ingame_continue') or (
                key == 'roll_the_dice' and 'pay_birthday' in self.menuitems['roll_the_dice'].type)):
            ...
            if self.menuitems[key].type == 'ingame_continue_PAY_RENT' or 'pay_birthday' in self.menuitems[key].type:
                ...
                if CUR.free_jail_cards:
                    for i in range(len(CUR.free_jail_cards)):
                        ...
                        self.objects['gamefield'].chests_and_chances[vid + 's'].append(
                            Globals.TEMP_VARS['free_jail_obj'])
            ...
            if type in ('roll_the_dice', 'roll_the_dice_to_exit_jail'):
                ...
                if type == 'roll_the_dice_to_exit_jail' and Globals.TEMP_VARS['dice1'] != Globals.TEMP_VARS['dice2']:
                    ...
                    self.labels['target_cell_owner'] = AlphaText(
                        Globals.TRANSLATION[54] + str(player.exit_jail_attempts), 'target_cell_owner', 1)
                    self.menuitems['ingame_continue'] = MenuItem(Globals.TRANSLATION[49],
                                                                 'ingame_continue_doesnt_exit_jail', 'ingame_main', 5)
                ...
                self.menuitems['ingame_push_to_auction_accept'] = MenuItem(Globals.TRANSLATION[82],
                                                                           'ingame_push_to_auction_accept',
                                                                           'ingame_main', 4)
                self.menuitems['return'] = MenuItem(Globals.TRANSLATION[64], 'return_player_on_a_new_cell',
                                                    'ingame_main', 5)
                ...
                if Globals.TEMP_VARS['cur_turn'] == len(Globals.PLAYERS) - 1:
                    ...
                    temp_range = range(Globals.TEMP_VARS['cur_turn'] + 1, len(Globals.PLAYERS)) + range(
                        Globals.TEMP_VARS['cur_turn'] + 1)
                else:
                    temp_range = list(range(Globals.TEMP_VARS['cur_turn'] + 1, len(Globals.PLAYERS))) + [0]
            ...
            elif type == 'property_management_input_ACCEPT' and self.menuitems['accept'].text.color == Globals.COLORS[
                'white']:
                ...
                if old_buildings != new_buildings:
                    ...
                    if Globals.TEMP_VARS['build_style']:
                        for cell in self.objects['gamefield'].cells:
                            if cell.group not in ('railroad', 'service') and cell.group in Globals.TEMP_VARS[
                                'RErender_groups'] and cell.number != cell_obj.number:
                    ...
                    for i in TEMP:
                        self.menuitems['fieldcell_' + str(i)].tooltip.RErender(
                            self.objects['gamefield'].cells[i].buildings + 1)
                    self.objects['gamefield'].RErender_fieldcell_groups()
                ...
                if 'cur_field_owner' not in Globals.TEMP_VARS.keys() and 'ingame_bankruptcy_10' not in self.menuitems.keys():
                    Globals.TEMP_VARS['cur_field_owner'] = self.objects['gamefield'].cells[
                        Globals.PLAYERS[Globals.TEMP_VARS['cur_turn']].cur_field].owner
                if 'pay_birthday' not in Globals.TEMP_VARS.keys():
                    self.disable_central_labels()
                    ...
                    for i in range(len(Globals.PLAYERS)):
                        if Globals.PLAYERS[i].name != temp_var['info'].name:
                            ...
                            if trader or trading_with:
                                counter += 1
                                self.menuitems['choose_player_to_trade_' + Globals.PLAYERS[i].name] = MenuItem(
                                    Globals.PLAYERS[i].name, 'enter_the_trade_menu_' + Globals.PLAYERS[i].name,
                                    'ingame_enter_the_trade_menu_' + Globals.PLAYERS[i].name, counter)
                ...
                if check_trading in (
                'trading_input_fields', 'trading_input_offer_money', 'trading_input_ask_for_money'):
                ...
                if type == 'trading_input_auction_bet':
                    ...
                    self.menuitems['return'] = MenuItem(Globals.TRANSLATION[64], 'return_auction_main', 'ingame_main',
                                                        6)
                self.clear_main_menu_entries(('return'))
                if 'error' in self.labels.keys():
                    self.labels.pop('error')
                ...
                if 'money' in type:
                    ...
                    text = Globals.TRANSLATION[83].replace('%', temp_var['order'][0].name).replace('@', temp_var[
                        'field'].NAME)
            ...
            elif type == 'trading_input_fields_ACCEPT' and self.menuitems['accept'].text.color == Globals.COLORS[
                'white']:
                ...
                if self.objects['trade_summary'].add_rm_fields(cell_obj):
                    ...
                    if cell_obj.owner not in (Globals.TEMP_VARS['trading']['trader']['info'].name,
                                              Globals.TEMP_VARS['trading']['tradingwith']['info'].name):
                        NUM = 74
                    else:
                        NUM = 106
                    self.show_or_rm_error_msg(True, NUM, 'ERROR_ingame', 'accept')
            elif type in ('trading_input_offer_money_ACCEPT', 'trading_input_ask_for_money_ACCEPT') and self.menuitems[
                'accept'].text.color == Globals.COLORS['white']:
            ...
            elif type == 'trading_input_auction_bet_ACCEPT' and self.menuitems['accept'].text.color == Globals.COLORS[
                'white']:
                temp_var = Globals.TEMP_VARS['auction']
                ...
                if temp_var:
                    ...
                    self.menuitems['ingame_bankruptcy_10'].update_text(
                        Globals.TRANSLATION[101] + str(CELL.buy_cost / 20))
                    self.menuitems['ingame_bankruptcy_110'].update_text(
                        Globals.TRANSLATION[102] + str(int((CELL.buy_cost / 2) * 1.1)))
                else:
                ...
                self.clear_main_menu_entries('return')
                self.labels['target_cell_info'] = AlphaText(
                    Globals.TEMP_VARS['trading']['tradingwith']['info'].name + Globals.TRANSLATION[77],
                    'trading_offer_request')
                self.menuitems['ingame_trading_ACCEPT_ALL'] = MenuItem(Globals.TRANSLATION[78],
                                                                       'ingame_trading_ACCEPT_ALL', 'ingame_main', 3)
            ...
            elif type and ('main_new_edit_player' in type or type == 'main_settings_player') and key == 'exit' and not \
            self.labels['name_MI'].symbols:
                ...
                if old != new:
                    ...
                    for i in range(old, new, int((new - old) / abs(new - old))):
                        ...
                        if new < old:
                            ...
                            self.menuitems.update({dictkey: MenuItem(Globals.PLAYERS[i].name,
                                                                     'main_new_edit_player_' + str(i),
                                                                     'main_new_playerlist', i)})
                            self.labels.update({dictkey: AlphaText('AI', 'newgame_playertype', i)})
                            self.cursor.add_rm_keys(True, dictkey, len(self.cursor.keys) - 2,
                                                    self.menuitems[dictkey].active_zone.move(0, self.menuitems[
                ...
                for lbl in self.labels.copy().keys():
                ...
                self.menuitems.update({'exit': MenuItem(u'×', 'main_main', 'from_game_return_to_menu'),
                                       ...
                                       'music': MenuItem((u'✖', u'✓')[int(Globals.SETTINGS['music'])],
                                                         'in_game_music_switch', 'music_and_sound_switches', 0),
                                       'sounds': MenuItem((u'✖', u'✓')[int(Globals.SETTINGS['sounds'])],
                                                          'in_game_sounds_switch', 'music_and_sound_switches', 1)})
                for cell in self.objects['gamefield'].cells:
                    if cell.group in list(range(1, 9)) + ['jail', 'railroad', 'service', 'skip']:
                        self.menuitems['fieldcell_' + str(cell.number)] = MenuItem('', 'onboard_select_cell_' + str(
                            cell.number), 'onboard_select_cell', cell.number)
                ...
                objects_to_move += [self.menuitems[key] for key in
                                    ('exit', 'show_menu', 'volume_level', 'music', 'sounds', 'show_prev_trades') if
                                    key in self.menuitems.keys()]
                objects_to_move += [self.labels[key] for key in ('volume_level', 'music', 'sounds', 'prev_trade') if
                                    key in self.labels.keys()]
            ...
            elif type == 'main_main' and 'show_menu' in self.menuitems.keys() and self.menuitems[
                'show_menu'].text.symbols == u'↓':
                return None
    ...
    def return_to_game_from_trading(self, type):
        ...
        if type == 'return_end_turn':
            ...
            CELL.step_indicator_visible = True
            self.labels['target_cell_trading_info'] = AlphaText(
                Globals.TRANSLATION[89].replace('%', Globals.TEMP_VARS['bankruptcy_RECIPIENT'].name),
                'target_cell_bankrupt_buyout', -2)
            self.labels['target_cell_info'] = AlphaText(CELL.NAME, 'target_cell_info', -1)
            self.menuitems['ingame_bankruptcy_10'] = MenuItem(Globals.TRANSLATION[101] + str(CELL.buy_cost / 20),
                                                              'ingame_bankruptcy_10', 'ingame_main', 3)
            self.menuitems['ingame_bankruptcy_110'] = MenuItem(
                Globals.TRANSLATION[102] + str(int((CELL.buy_cost / 2) * 1.1)), 'ingame_bankruptcy_110', 'ingame_main',
                4)
            ...
            if player.cur_field == 10 and player.exit_jail_attempts != None:
                ...
                if cell.owner and 'cur_field_owner' in Globals.TEMP_VARS.keys() and Globals.TEMP_VARS[
                    'cur_field_owner'] != cell.owner:
                    Globals.TEMP_VARS['xxx'] = cell.owner
    ...
    def return_into_prop_manage_choose_field(self):
        ...
        if not KEY in self.menuitems.keys() and CHANGES:
            ...
            self.cursor.add_rm_keys(True, KEY, 0, self.menuitems[KEY].active_zone.move(0,
                                                                                       self.menuitems[KEY].text.new_pos[
                                                                                           1] - self.menuitems[
                                                                                           KEY].text.rect.y).topleft)
    ...
    def clear_labels(self, exception):
        for key in self.labels.copy().keys():
    ...
    def save_step_indicators_state(self):
        Globals.TEMP_VARS['save_step_indicators_state'] = [cell.number for cell in self.objects['gamefield'].cells if
                                                           cell.step_indicator_visible]
        self.disable_step_indicators()
    ...
    def cancel_prop_manage(self):
        ...
        if temp:
            ...
            for cell in self.objects['gamefield'].cells:
                cell.a_little_number_visible = False
                if cell.number in Globals.TEMP_VARS['property'].keys() and cell.buildings != \
                        Globals.TEMP_VARS['property'][cell.number]:
    ...
    def make_stats_screen(self, current):
        ...
        if data[3]['score']:
            for i in range(3, len(data)):
                if data[i]['score']:
                    self.labels.update(
                        {'bestname' + str(i - 2): AlphaText(str(i - 2) + '. ' + data[i]['name'], 'stats_table_0', i),
                         'bestscore' + str(i - 2): AlphaText(
                             '  ' * (10 - len(str(data[i]['score']))) + str(data[i]['score']), 'stats_table_1', i),
    ...
    def make_settings_screen(self):
        self.menuitems = {'language': MenuItem(u'‹ ' + Globals.TRANSLATION[60] + u' ›', 'main_settings_language',
                                               'main_settings_left_MI', 0),
                          'player': MenuItem(Globals.PLAYERS[0].name, 'main_settings_player', 'main_settings_player',
                                             0),
                          'hotkeys': MenuItem(
                              u'‹ ' + Globals.TRANSLATION[18 - int(Globals.SETTINGS['hotkeys'])] + u' ›',
                              'main_settings_hotkeys', 'main_settings_left_MI', 2),
                          'music': MenuItem(u'‹ ' + Globals.TRANSLATION[18 - int(Globals.SETTINGS['music'])] + u' ›',
                                            'main_settings_music', 'main_settings_left_MI', 3),
                          'sounds': MenuItem(u'‹ ' + Globals.TRANSLATION[18 - int(Globals.SETTINGS['sounds'])] + u' ›',
                                             'main_settings_sounds', 'main_settings_left_MI', 4),
                          'volume': MenuItem('', 'main_settings_volume_SELECTOR', 'main_settings_left_MI', 5),
                          'build_style': MenuItem(
                              u'‹ ' + Globals.TRANSLATION[18 - int(Globals.SETTINGS['build_style'])] + u' ›',
                              'main_settings_build_style', 'main_settings_left_MI', 6),
        ...
        if not Globals.SETTINGS['block']:
            self.menuitems.update({'fav_game': MenuItem(
                u'‹ ' + Globals.TRANSLATION[5 + int(Globals.SETTINGS['fav_game'])] + u' ›', 'main_settings_fav_game',
                'main_settings_left_MI', 7)})
    ...
    def make_playersettings_screen(self):
        self.menuitems = {
            'name': MenuItem(Globals.PLAYERS[Globals.TEMP_VARS['edit_player']].name, 'main_settings_player_name',
                             'main_settings_player', 0),
    ...
    def init_avail_colors_and_names(self):
        ...
        Globals.TEMP_VARS['avail_names'] = read_file(
            Globals.DIRS['translations'] + Globals.SETTINGS['language'] + '/names')
        for player in Globals.PLAYERS:
    ...
    def restore_step_indicators_state(self):
        ...
        if 'save_step_indicators_state' in Globals.TEMP_VARS.keys():
            ...
            self.objects['gamefield'].cells[
                Globals.PLAYERS[Globals.TEMP_VARS['cur_turn']].cur_field].step_indicator_visible = True
    ...
    def show_property_management_menuitems(self, number, condition=True):
        if condition:
            ...
            if check_if_player_owns_fieldcells(trader_name):
                self.menuitems['manage_property'] = MenuItem(Globals.TRANSLATION[62], 'enter_the_property_management',
                                                             'ingame_main', number + 1)
    ...
    def make_return_button_for_property_management(self):
        ...
        self.menuitems['return'] = MenuItem(Globals.TRANSLATION[(64, 93)['property' in Globals.TEMP_VARS.keys()]],
                                            back_type, 'ingame_main', 8)
    ...
    def show_main_trading_menu(self):
        ...
        if trader or trading_with:
            counter += 1
            self.menuitems['trading_input_fields'] = MenuItem(Globals.TRANSLATION[65], 'trading_input_fields',
                                                              'ingame_main', counter)
        for i in range(2):
            counter += 1
        ...
        for key in ('trader', 'tradingwith'):
            for i in range(len(temp_var[key]['info'].free_jail_cards)):
                ...
                self.menuitems[operation + str(i)] = MenuItem(text, operation + str(i),
                                                              'ingame_main_trading_jails_' + key, counter)
    ...
    def show_trading_OFFER_ALL_button(self, cursor_key_just_add=False):
        if self.check_trading_accept_ability('trader') and self.check_trading_accept_ability(
                'tradingwith') and 'text_cursor' not in self.objects.keys():
            if 'accept_ALL' not in self.menuitems.keys():
                self.menuitems['accept_ALL'] = MenuItem(Globals.TRANSLATION[76], 'ingame_trading_OFFER_ALL',
                                                        'ingame_main', 7)
                if cursor_key_just_add:
                    self.cursor.add_rm_keys(True, 'accept_ALL', len(self.cursor.keys) - 1,
                                            self.menuitems['accept_ALL'].active_zone.move(0, self.menuitems[
                                                'accept_ALL'].text.new_pos[1] - self.menuitems[
                                                                                              'accept_ALL'].text.rect.y).topleft)
    ...
    def create_trading_input_spec_objects(self, KEY):
        ...
        if self.labels[KEY].symbols and (
                'trading_input' in KEY or KEY == 'property_management_input') and 'accept' not in self.menuitems.keys():
            ...
            self.cursor.add_rm_keys(True, 'accept', 0, self.menuitems['accept'].active_zone.move(0, self.menuitems[
                'accept'].text.new_pos[1] - self.menuitems['accept'].text.rect.y).topleft)
    ...
    def create_prop_management_cell_state_objects(self, CELL):
        ...
        if CELL.owner == player.name:
            ...
            if 'state_selector' in self.cursor.keys:
                self.cursor.add_rm_keys(False, 'state_selector')
            self.cursor.add_rm_keys(True, 'state_selector', 0, self.menuitems['state_selector'].active_zone.move(0,
                                                                                                                 self.menuitems[
                                                                                                                     'state_selector'].text.new_pos[
                                                                                                                     1] -
                                                                                                                 self.menuitems[
                                                                                                                     'state_selector'].text.rect.y).topleft)
        else:
    ...
    def check_error(self, type):
        ...
        if type in ('trading_input', 'property_management_input'):
            ...
            if obj and int(obj) > MAX:
                self.show_or_rm_error_msg(True, data[0], data[1], data[2])
            elif (obj and int(obj) <= MAX and 'error' in self.labels.keys()) or (
                    not obj and 'error' in self.labels.keys()):
                self.show_or_rm_error_msg(False, data[0], data[1], data[2])
    ...
    def show_or_rm_error_msg(self, SHOW, lbl_translation_num, lbl_type, menuitem_key):
        if SHOW:
            self.labels['error'] = AlphaText(Globals.TRANSLATION[lbl_translation_num], lbl_type,
                                             int('trading_input_fields' in self.menuitems.keys()))
    ...
    def error_msg_money_limits(self, key):
        if key == 'accept_all_prop_management':
            ...
            condition = self.cursor and self.cursor.uCOLOR == 'red27' and self.cursor.uCondition and key in (
            'buy_a_cell', 'ingame_continue', 'auction_up_bet', 'pay_money_to_exit_jail', 'roll_the_dice',
            'ingame_bankruptcy_10', 'ingame_bankruptcy_110')
    ...
    def check_doubles_for_players(self):
        for i in range(len(Globals.PLAYERS) - 1):
            for j in range(i + 1, len(Globals.PLAYERS)):
                if Globals.PLAYERS[i].color == Globals.PLAYERS[j].color or Globals.PLAYERS[i].name == Globals.PLAYERS[
                    j].name:
                    return True
            ...
            if old_buildings != new_buildings:
                ...
                if CELL.group in range(9):
                    MONEY += (old_buildings - new_buildings) * CELL.build_cost / (
                                1 + 1 * int(new_buildings < old_buildings))
    ...
    def change_player(self, bankrupt=None):
        ...
        if len(Globals.PLAYERS) > 1:
            ...
            if self.cursor:
                self.clear_main_menu_entries()
            self.labels['target_cell_name'] = AlphaText(Globals.TRANSLATION[107] + Globals.PLAYERS[0].name,
                                                        'target_cell_name', 0)
            self.labels['target_cell_name'].change_color(Globals.PLAYERS[0].color)
            self.labels['target_cell_info'] = AlphaText(
                Globals.TRANSLATION[109] + str(count_player_funds(Globals.PLAYERS[0])), 'target_cell_info', 2)
    ...
    def new_turn(self):
        ...
        if player.human:
            if self.cursor:
                self.clear_main_menu_entries()
            if player.cur_field == 10 and player.exit_jail_attempts != None:
                self.menuitems['pay_money_to_exit_jail'] = MenuItem(
                    Globals.TRANSLATION[55] + str(self.objects['gamefield'].cells[10].buy_cost),
                    'pay_money_to_exit_jail', 'ingame_main', int(player.exit_jail_attempts > 0))
                if player.exit_jail_attempts:
                    self.menuitems['roll_the_dice'] = MenuItem(
                        Globals.TRANSLATION[43] + ' (' + str(player.exit_jail_attempts) + ')',
                        'roll_the_dice_to_exit_jail', 'ingame_main', 0)
                if player.free_jail_cards:
                    self.menuitems['use_card_to_exit_jail'] = MenuItem(
                        Globals.TRANSLATION[57] + ' (' + str(len(player.free_jail_cards)) + ')',
                        'use_card_to_exit_jail', 'ingame_main', 1 + int(player.exit_jail_attempts > 0))
            else:
                self.menuitems['roll_the_dice'] = MenuItem(Globals.TRANSLATION[43], 'roll_the_dice', 'ingame_main', 0)
            number = len([key for key in ('pay_money_to_exit_jail', 'roll_the_dice', 'use_card_to_exit_jail') if
                          key in self.menuitems.keys()])
    ...
    def player_on_a_new_cell(self, cell):
        ...
        if cell.group in ('jail', 'skip', 'gotojail', 'start', 'income', 'tax', 'chest', 'chance'):
            ...
            if group in ('chest', 'chance'):
                ...
                property_management_condition = (obj.type == 'income' and obj.modifier[0] < 0) or obj.type in (
                'pay_each', 'repair')
            else:
                property_management_condition = cell.group == 'tax'
            if group in ('chest', 'chance') and self.objects['gamefield'].chests_and_chances[group + 's'][
                0].type == 'goto_jail':
                self.menuitems['ingame_continue'] = MenuItem(Globals.TRANSLATION[49], 'ingame_continue_gotojail',
                                                             'ingame_main', 5)
                      ...
                      or (group in ('chest', 'chance')
                          and ((obj.type == 'income'
                               ...
                               or (obj.type == 'pay_each'
                                   and check_bankrupt(PLAYER))))):
                    addition += Globals.TRANSLATION[100]
                self.menuitems['ingame_continue'] = MenuItem(Globals.TRANSLATION[49] + addition,
                                                             'ingame_continue_' + group, 'ingame_main', 5)
            Globals.TEMP_VARS['take_chance_when_player_is_on_chest'] = False
        else:
            if cell.owner:
                addition = ''
                    ...
                    if 'cur_field_owner' not in Globals.TEMP_VARS.keys():
                        if cell.group == 'service':
                            if cell.step_indicator_visible:
                                ...
                                state = 10
                            Globals.TEMP_VARS['MUST_PAY'] = (Globals.TEMP_VARS['dice1'] + Globals.TEMP_VARS[
                                'dice2']) * state
                        else:
                            ...
                            if (not cell.buildings and self.objects['gamefield'].groups_monopolies[cell.group]) or (
                                    cell.group == 'railroad' and 'double_rails' in Globals.TEMP_VARS.keys()) or (
                                    cell.group == 'railroad' and not cell.step_indicator_visible and
                                    self.objects['gamefield'].chests_and_chances['chances'][0].type != 'goto' and
                                    self.objects['gamefield'].chests_and_chances['chances'][0].modifier != [5]):
                                Globals.TEMP_VARS['MUST_PAY'] = Globals.TEMP_VARS['MUST_PAY'] * 2
                                if cell.group == 'railroad':
                                    Globals.TEMP_VARS['double_rails'] = True
                    self.labels['target_cell_info'] = AlphaText(
                        Globals.TRANSLATION[50] + str(Globals.TEMP_VARS['MUST_PAY']), 'target_cell_info', 2)
                ...
                Globals.TEMP_VARS['MUST_PAY'] = cell.buy_cost
                self.menuitems.update({'buy_a_cell': MenuItem(Globals.TRANSLATION[47] + str(cell.buy_cost) + ')',
                                                              'ingame_buy_a_cell', 'ingame_main', 5),
                                       'cell_to_an_auction': MenuItem(Globals.TRANSLATION[48],
                                                                      'ingame_cell_to_an_auction', 'ingame_main', 6)})
        ...
        self.cursor.screen_switched(self.menuitems, ('ingame_buy_or_auction', 'ingame_continue')[
            'ingame_continue' in self.menuitems.keys()])
    ...
    def pay_birthday_next_player(self):
        if Globals.TEMP_VARS['pay_birthday']:
            ...
            if check_bankrupt(player):
                addition = Globals.TRANSLATION[100]
            self.menuitems.update({'roll_the_dice': MenuItem(
                Globals.TRANSLATION[55] + str(Globals.TEMP_VARS['MUST_PAY']) + addition, 'pay_birthday_' + player.name,
                'ingame_main', 3)})
    ...
    def auction_next_player(self):
        ...
        if len(temp_var['order']) > 1 or (len(temp_var['order']) and not temp_var['bet']):
            self.labels['target_cell_info'] = AlphaText(Globals.TRANSLATION[89].replace('%', temp_var['order'][0].name),
                                                        'auction_info', -3)
            ...
            self.menuitems.update({'auction_up_bet': MenuItem(text, 'trading_input_auction_bet', 'ingame_main', 1),
                                   'auction_refuse': MenuItem(Globals.TRANSLATION[85], 'auction_refuse', 'ingame_main',
                                                              number)})
            ...
            if 'bankruptcy_fields_changing' in Globals.TEMP_VARS.keys():
                ...
                type = ('return_end_turn', 'return_new_turn')[
                    Globals.TEMP_VARS['dice1'] == Globals.TEMP_VARS['dice2'] and Globals.PLAYERS[
                        Globals.TEMP_VARS['cur_turn']].exit_jail_attempts == None]
                self.return_to_game_from_trading(type)
    ...
    def bankruptcy_fields_buyout(self, key):
        temp_var = Globals.TEMP_VARS['bankruptcy_fields_changing']
        if temp_var and self.menuitems[key].type == 'ingame_continue_PAY_RENT' or 'pay_birthday' in self.menuitems[
            key].type:
            if len(Globals.PLAYERS) > 1:
                CELL = self.objects['gamefield'].cells[temp_var[0]]
                self.clear_main_menu_entries()
                ...
                self.menuitems['ingame_bankruptcy_10'] = MenuItem(Globals.TRANSLATION[101] + str(CELL.buy_cost / 20),
                                                                  'ingame_bankruptcy_10', 'ingame_main', 3)
                self.menuitems['ingame_bankruptcy_110'] = MenuItem(
                    Globals.TRANSLATION[102] + str(int((CELL.buy_cost / 2) * 1.1)), 'ingame_bankruptcy_110',
                    'ingame_main', 4)
                self.show_property_management_menuitems(5)
                self.cursor.screen_switched(self.menuitems, 'bankruptcy_buyout')
            else:
    ...
    def DEBUGGER_prop_management_money_changes(self):
        for i in Globals.TEMP_VARS['prop_manage_CHANGED'].keys():
            if i != 'TOTAL':
                print(str(i) + '   : ' + str(Globals.TEMP_VARS['prop_manage_CHANGED'][i][0]) + '   ' + str(
                    Globals.TEMP_VARS['prop_manage_CHANGED'][i][1]) + '   ' + str(
                    Globals.TEMP_VARS['prop_manage_CHANGED'][i][2]))
        print('-----------------------------------------------')
