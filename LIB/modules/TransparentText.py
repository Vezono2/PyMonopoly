    ...
    def init_for_group(self, number):
        ...
        if 'SELECTOR' in self.group:
            ...
            self.font = Globals.FONTS['ume_8']
                ...
                'target_cell_owner', 'target_cell_info', 'target_cell_bankrupt_buyout', 'birthday_info', 'auction_info',
                'trade_summary_trader_splitter', 'prop_manage_summary_splitter', 'trading_offer_request',
                'last_trade_info') or 'gamelog_message' in self.group:
        ...
        elif self.group[:4] in ('inga', 'main') or self.group in (
                'stats_game_name', 'main_settings_left_MI', 'main_settings_player', 'target_cell_name'):
            self.font = Globals.FONTS['ubuntu_24']
        elif self.group == 'stats_common':
            self.font = Globals.FONTS['ubuntu_20']
        elif self.group in (
                'APPVERSION', 'authors', 'stats_switch', 'stats_bests', 'settings_left', 'volume_in_game_lbl',
                'trade_summary_trader_name', 'trade_summary_tradingwith_name', 'prop_manage_summary_name',
                'auction_cur_bet',
                'bank_property1') or 'stats_table' in self.group or 'ERROR' in self.group:
            ...
            self.font = Globals.FONTS['ume_16']
                ...
                'stats_latest', 'a_little_cell_number', 'prop_manage_summary_fields', 'auction_player_order',
                'bank_property2'):
        ...
        if self.group == 'target_cell_owner':
            self.color = Globals.main_scr.objects['gamefield'].cells[
                Globals.PLAYERS[Globals.TEMP_VARS['cur_turn']].cur_field].color
            ...
            self.color = Globals.TEMP_VARS['trading'][self.group.split('_')[2]]['info'].color
                ...
                'authors', 'stats_switch', 'from_game_return_to_menu', 'show_menu', 'show_prev_trades',
                'pl_money_info'):
        ...
        elif self.group[:28] in (
                'ingame_main', 'ingame_dices', 'ingame_enter_the_trade_menu_', 'ingame_main_trading_jails_tr'):
            self.x = 'center'
            self.x_offset = 0
    ...
    def init_new_pos(self):
        if self.group[:12] in ('target_cell_', 'auction_info', 'auction_cur_'):
            self.new_pos = (self.rect.x, self.rect.y - 70)
                ...
                'trading_offer_request', 'birthday_info', 'cell_state_SELECTOR', 'auction_player_order') or self.group[
                                                                                                            :5] in (
                'main_', 'stats', 'setti', 'newga', 'ingam'):
