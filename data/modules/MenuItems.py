...
class MenuItem:
    ...
    def init_for_type(self):
        # --- Hotkeys
        if self.type in (
                'main_settings_language', 'main_settings_hotkeys', 'main_settings_music', 'main_settings_sounds',
                'main_settings_build_style', 'main_settings_fav_game', 'stats_switch', 'main_new_game_switch'):
    ...
    def render(self, state):
        ...
        if self.group[:4] not in ('main', 'inga'):
            ...
            if self.tooltip and ((Globals.SETTINGS['hotkeys'] and self.HOTKEYS) or (not self.HOTKEYS)) and not (
                    self.group == 'onboard_select_cell' and Globals.main_scr.objects['gamefield'].pos[1] != 70):
    ...
    def action(self, key):
        ...
        if 'SELECTOR' in self.type:
            ...
            SYSEXIT()
                ...
                'main_settings_music', 'main_settings_sounds', 'main_settings_hotkeys', 'main_settings_build_style'):
        ...
        if self.number == 10:
            ...
            if jailcards:
                for player in jailcards:
                    line += 1
                    self.text.blit(font.render(Globals.TRANSLATION[56] + player.name, True, player.color),
                                   (0, 18 * line))
        else:
            ##--- Buy cost
            if self.number != 20:
            ...
            ##--- Rentlabels
            if CELL.group in list(range(1, 9)) + ['railroad', 'service']:
                start_string = 6
                ...
                for i in range(count):
                    ...
                    self.text.blit(font.render((Globals.TEMP_VARS['rentlabels'][i + start_string]), True, color),
                                   (0, 45 + i * 15))
                    if cell_state == 1 and i == 0 and CELL.group not in (None, 'railroad', 'service') and \
                            Globals.main_scr.objects['gamefield'].groups_monopolies[CELL.group]:
                ...
                for i in range(2):
                    ...
                    self.text.blit(font.render(string, True, color),
                                   (180 - font.size(string)[0], 60 + (count + i) * 15))
...
class MenuSelector():
    def __init__(self, type):
        ...
        if 'volume_SELECTOR' in type:
            ...
            itemcount_end = 2 + (5 - int(Globals.TEMP_VARS['cur_game'])) * (group in range(9)) * bool(
                gamefield.groups_monopolies[group])
...
class MainCursor(Cursor):
    ...
    def make_keys(self, type):
        if type == 'ingame_continue':
            ...
            array = ['trading_input_fields', 'trading_input_offer_money', 'trading_input_ask_for_money',
                     'trading_offer_free_jail', 'trading_ask_for_free_jail']
            ...
            self.keys_generator(
                ('roll_the_dice', 'pay_money_to_exit_jail', 'use_card_to_exit_jail', 'trade', 'manage_property'))
    ...
    def RErender(self):
        ...
        if self.uCOLOR:
            if self.u_length < self.rect.w / 2 - 15:
                self.u_length += 10
                self.draw_rect()
                pygame.draw.line(self.surf, Globals.COLORS[self.uCOLOR],
                                 (self.rect.w / 2 - self.u_length, self.rect.h - 1),
                                 (self.rect.w / 2 + self.u_length, self.rect.h - 1), 1)
        else:
            self.u_length = 0
            self.draw_rect()
    ...
    def render(self, state):
        if state:
            if self.u_length < self.rect.w / 2 - 15:
                self.u_length += 2
                self.draw_rect()
                pygame.draw.line(self.surf, self.u_color, (self.rect.w / 2 - self.u_length, self.rect.h - 1),
                                 (self.rect.w / 2 + self.u_length, self.rect.h - 1), 1)
            Cursor.render(self)
        elif self.u_length:
            self.u_length = 0
...
class CurTurnHighlighter(Cursor):
    def __init__(self, menuitems):
        self.verts = [menuitems['player_' + Globals.PLAYERS[i].name].active_zone[1] for i in
                      range(len(Globals.PLAYERS))]
        Cursor.__init__(self, 80, pygame.Rect((menuitems['player_' + Globals.PLAYERS[
            Globals.TEMP_VARS['cur_turn']].name].active_zone[0] - 162, self.verts[0]), (200, 39)))
        self.draw_rect()
