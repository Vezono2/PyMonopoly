# -*- coding: utf-8 -*-
from . import Globals, FieldCellsData
import pygame
...
class GameField():
    ...
    def render_cell_numbers(self, type):
        if type == 'trade':
            ...
            cells = [cell for cell in self.cells if cell.owner in traders and (
                        (cell.group in ('railroad', 'service') and cell.buildings > -1) or not cell.buildings)]
...
class FieldCell():
    def __init__(self, group_symbol, group_colors, build_costs, number, size, pos):
        ...
        if number in Globals.TEMP_VARS['onboard_text']['onboard'].keys():
            self.onboard_text = Globals.FONTS['ubuntu_16'].render(Globals.TEMP_VARS['onboard_text']['onboard'][number],
                                                                  True, Globals.COLORS['black'])
        else:
...
class GameLog(InfoWindow):
    ...
    def add_message(self, type):
        if type in ('roll_the_dice', 'chest_goto'):
            text = Globals.GAMELOG_TRANSLATION[(2, 10)[type == 'chest_goto']].replace('%', str(
                Globals.PLAYERS[Globals.TEMP_VARS['cur_turn']].cur_field))
            ...
            text = Globals.GAMELOG_TRANSLATION[3 + 10 * (type[16:] == 'income')].replace('%', str(
                Globals.TEMP_VARS['MUST_PAY'])[(type[16:] == 'tax'):])
        elif type == 'ingame_continue_PAY_RENT':
            text = Globals.GAMELOG_TRANSLATION[4].replace('%', str(Globals.TEMP_VARS['MUST_PAY'])).replace('^',
                                                                                                           Globals.main_scr.objects[
                                                                                                               'gamefield'].cells[
                                                                                                               Globals.PLAYERS[
                                                                                                                   Globals.TEMP_VARS[
                                                                                                                       'cur_turn']].cur_field].owner)
        elif type == 'ingame_buy_a_cell':
            text = Globals.GAMELOG_TRANSLATION[5].replace('%', str(
                Globals.PLAYERS[Globals.TEMP_VARS['cur_turn']].cur_field)).replace('^',
            ...
            text = '- ' + Globals.GAMELOG_TRANSLATION[(3, 7)[Globals.TEMP_VARS['MUST_PAY'] > 0]].split()[
                1] + ' $' + str(Globals.TEMP_VARS['MUST_PAY']).lstrip('-')
            ...
            text = Globals.GAMELOG_TRANSLATION[8].replace('%', str(
                Globals.PLAYERS[Globals.TEMP_VARS['cur_turn']].exit_jail_attempts))
            ...
            field = type[18:]
            temp_var = Globals.TEMP_VARS['prop_manage_CHANGED'][int(field)]
            text = Globals.GAMELOG_TRANSLATION[21].replace('%', field) + str(temp_var[2] + int(
                int(temp_var[0] < 0) * 1.1 * (Globals.main_scr.objects['gamefield'].cells[int(field)].buy_cost / 2)))[
                                                                         1:]
        elif type[:18] == 'prop_manage_debuil':
            field = type[20:]
            temp_var = Globals.TEMP_VARS['prop_manage_CHANGED'][int(field)]
            text = Globals.GAMELOG_TRANSLATION[22].replace('%', field) + str(
                temp_var[2] - int(temp_var[1] < 0) * Globals.main_scr.objects['gamefield'].cells[
                    int(field)].buy_cost / 2)
            ...
            text = Globals.GAMELOG_TRANSLATION[11 + (type == 'pay_each')].replace('%',
                                                                                  str(Globals.TEMP_VARS['MUST_PAY']))
...
class PropManageSummary(InfoWindow):
    ...
    def add_field(self, num):
        ...
        state = []
        for i in range(2):
            if Globals.main_scr.objects['gamefield'].cells[num].group in ('railroad', 'service') and temp_var[num][
                i] > -1:
                state.append(pics[1])
            else:
    ...
    def upd_TOTAL(self):
        ...
        if temp_var > 0:
            ...
            color = 'white'
        elif check_cur_prop_management().money < -temp_var:
            color = 'light_red'
        else:
            color = 'orange'
